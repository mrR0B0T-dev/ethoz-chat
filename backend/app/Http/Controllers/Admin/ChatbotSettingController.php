...
class ChatbotSettingController extends Controller
{
    ...
    /**
     * Pratinjau system prompt yang diterima AI untuk sebuah peran.
     * Membaca data yang SUDAH tersimpan, jadi simpan pengaturan dulu.
     */
    public function preview(Request $r, ChatbotService $service)
    {
        $data = $r->validate([
            'role' => 'nullable|in:staff,hr,manager,admin',
        ]);

        // User tiruan yang tidak disimpan ke database — hanya perannya yang
        // dipakai ChatbotService untuk memfilter basis pengetahuan.
        $preview = new User;
        $preview->role = $data['role'] ?? 'staff';

        return response()->json(['prompt' => $service->buildSystemPrompt($preview)]);
    }
}
